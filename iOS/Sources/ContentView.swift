...
struct EntryEditorView: View {
    ...
    init(entry: LogEntry?) {
        self.entry = entry
        _spot = State(initialValue: entry?.spot ?? "")
        _clarity = State(initialValue: entry?.clarity ?? "")
        ...
        NavigationStack {
            Form {
                Section("Spot") {
                    TextField("Spot", text: $spot)
                        ...
                        .accessibilityIdentifier("fieldSpot")
                }
                Section("Clarity") {
                    TextField("Clarity", text: $clarity)
                        .focused($focusedField, equals: .f1)
                        .accessibilityIdentifier("fieldClarity")
                }
                Section("Notes") {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var updated = entry ?? LogEntry(spot: "", clarity: "", notes: "")
                        updated.spot = spot
                        updated.clarity = clarity
                        updated.notes = notes
                        if entry == nil {
                            store.add(updated)
                        } else {
